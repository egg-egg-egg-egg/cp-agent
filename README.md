...
- 🔍 **原题查重**：内置 24000+ 题库（Codeforces + 洛谷），hybrid 检索召回 + 独立 LLM 裁判判定"是否同一题目模型"，判撞题即强制换题；生成成功的题自动入库，批量出题不会自己撞自己
- 🕵️ **独立验题（可选 --cross-check）**：另一模型只看题面盲解，与标程全测试点比对，捕获"标程和暴力错得一致"的同源错误
- 📏 **难度校准（可选 --difficulty-review）**：独立评审估计 CF rating，与标称偏差 >300 时警告
